...
const hook = new Discord.WebhookClient('666196758258384917', 'MuBpdXTGWkYwcY6PPp8ZD-TtRlp6aGhPYnqQrrgOK38NpSbZtjyWG8ghbTy6-YgNHIWE');
const gunei = new Discord.WebhookClient('666906780701294592', 'Qp-UPrRzhZUAx9FVEnbJTlvDszmJJ8mN989SZv_V8PYSgOtH_oHbKtIgzAbRrz8804hw');
var random = 0;
var boolcount = true;
var p1 = 0;
var f = 0;
...
function counttime(time) {
    ...
    ms = remaining;
    return day + "일 " + hour + "시간 " + minute + '분 ' + second + "초 " + ms + "ms";
}
function clean(text) {
...
function type(text) {
    if (text.toLowerCase().startsWith('promise')) {
        ...
        return '배열';
    } else if (typeof (text).toLowerCase() === 'undefined' || typeof (text).toLowerCase() === null) {
        return 'NULL(값이 없음)';
    }
    else {
        return text;
...
var afktime = null;
var pauthor = null;
const bug = new Discord.WebhookClient('670905572295704586', 'Jkt2dX0Ckdq_X4nhJqgxJhxt26bafBSC8LnyF4xFyJaK6c1ZRpbtfPePoE6RrTYe2KWL');
var ntitle = null;
var ncont = null;
var gongji = 0;
var poptions = [];
...
client.on('ready', () => {
    ...
    console.log('로그인됨');
    nowgame = "'/도움'을 입력해주세요! | ditto7890#8948님이 봇 개발에 많이 기여(?)하셨습니다.";
    nowstatus = "online";
    function getping() {
    ...
    if (boolcount) {
        client.user.setPresence({
            status: "online",
            game: {
                name: "'/도움' 명령어 입력 | " + client.guilds.size + "개의 서버에 참여중 | " + client.users.size + "명의 사용자와 함께하는 중",
                type: "PLAYING"
            }
        });
    } else {
...
client.on('message', async message => {
    try {
        ...
        if (message.content === '//공지' && message.author.id === '647736678815105037') {
            gongji = 1;
        ...
        else if (message.content === '//공지 초기화' && message.author.id === '647736678815105037') {
            notice.channels = [];
            fs.writeFile("./notice.json", JSON.stringify(notice), (err) => {
                if (err) console.log(err)
            });
            ...
            if (notice.channels != []) {
                ...
                for (i = 0; i < notice.channels.length; i++){
                    ...
                    if (test) {
                        ...
                        embed2.setDescription(message.author.tag + '님에 의해 작성된 공지\n현재 전송 완료율\n' + i + '/' + notice.channels.length + '개 채널 전송 완료\n현재 전송중인 채널 정보\n' + client.channels.get(notice.channels[i]).name + '(' + notice.channels[i] + ')');
                ...
                const embed3 = new Discord.RichEmbed()
                    .setTitle('공지 전송 완료!')
                    .setDescription(message.author.tag + '님이 작성한 공지를 전송했습니다.')
        ...
        else if (v === 1 && message.author.id === '647736678815105037' && message.channel.id === '666906733754712095') {
            nowgame = message.content;
            if (boolcount) {
                client.user.setPresence({
                    status: nowstatus,
                    game: {
                        name: nowgame + " | " + client.guilds.size + "개의 서버에 참여중 | " + client.users.size + "명의 사용자와 함께하는 중",
                        type: "PLAYING"
                    }
                });
            } else {
        ...
        else if (message.content === '//count false' && message.author.id === '647736678815105037' && message.channel.id === '666906733754712095') {
            boolcount = false;
            client.user.setPresence({
                status: nowstatus,
                game: {
        ...
        else if (message.content === '//count true' && message.author.id === '647736678815105037' && message.channel.id === '666906733754712095') {
            boolcount = true;
            client.user.setPresence({
                status: nowstatus,
                game: {
        ...
        if (message.content.startsWith("/eval ")) {
            if (message.author.id != '647736678815105037' && message.author.id != '657958609526849536') return;
            ...
            try {
                ...
                const embed = new Discord.RichEmbed()
                    .setTitle('eval 결과')
                    .setDescription('~~이건 제가 ditto7890#8948님 도움 없이 혼자서 만든 코드입니다.~~')
                    .addField('입력 코드', '```js\n' + message.content.substr(6) + '\n```')
                    .addField('결과', '```js\n' + clean(evaled) + '\n```')
                    .addField('결과 자료형', '```js\n' + type(clean(evaled)) + '\n```')
                    .setColor(0x00ff00)
                ...
                const embed = new Discord.RichEmbed()
                    ...
                    .addField('에러 내용', err)
                    .setThumbnail('https://cdn.discordapp.com/attachments/665169857972797440/670904480388218900/unknown.png');
                const embed2 = new Discord.RichEmbed()
                    .setTitle('eval 오류')
                    .setDescription('~~이건 제가 ditto7890#8948님 도움 없이 혼자서 만든 코드입니다.~~')
                    .addField('입력 코드', '```js\n' + message.content.substr(6) + '\n```')
        ...
        else if (s === 1 && message.author.id === '647736678815105037' && message.channel.id === '666906733754712095') {
            ...
            const embed = new Discord.RichEmbed()
                ...
                .setURL('https://discordapp.com/api/oauth2/authorize?client_id=657958609526849536&permissions=8&scope=bot', '기본 권한으로 초대하기')
                .addField('권한 직접 설정(아이디: 657958609526849536', 'https://mswgen.github.io/discordbotlink')
            message.channel.send(embed);
            ...
            if (true) {
                ...
                p1 = 0;
                pcount = 2;
                message.reply('2번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 3;
                message.reply('3번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
        }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 4;
                message.reply('4번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
        }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 5;
                message.reply('5번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
        }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 6;
                message.reply('6번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
        }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 7;
                message.reply('7번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 8;
                message.reply('8번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 9;
                message.reply('9번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
            ...
            if (message.content != 'esc') {
                ...
                p1 = 0;
                pcount = 10;
                message.reply('10번째 투표 항목을 입력하세요(취소하려면 esc를 입력해주세요)')
            } else {
                poll(message.channel);
            }
        }
        ...
        */
            ...
            var gu = '';
            const embed = new Discord.RichEmbed()
                .setTitle('mswgen봇이 있는 서버 리스트(총 ' + client.guilds.size + '개)')
                .setColor(0x00ff00)
                .setThumbnail('https://playentry.org/uploads/profile/5c/ff/avatar_5cff3addbc5c91571a8b8ecd.png?1578276384708')
                .setFooter(message.author.tag, message.author.avatarURL)
                .setTimestamp()
        ...
        else if (message.content === "/내 정보" || message.content === "/내정보") {
            ...
            if (message.guild.verificationLevel === 0) {
                ...
                verifylv = '높음(Discord 이메일 인증 + 가입 5분 경과 + 서버 참가 10분 경과 필요)';
            } else if (message.guild.verificationLevel === 4) {
                verifylv = '매우 높음(DIscord 이메일, 휴대폰 인증 + 가입 5분 경과 + 서버 참가 10분 경과 필요)';
            }
            ...
            const embed = new Discord.RichEmbed()
                ...
                .addField('서버 관리에 2단계 인증 필요 여부', message.guild.mfaLevel, true)
            ...
            const embed = new Discord.RichEmbed()
                ...
                .addField('봇 제작자', 'Skype와 Teamspeak랑 이별할 시간이에요#5458(참고로 원래 닉네임은 mswgen#5458입니다.)')
            ...
            message.channel.send(link('https://www.youtube.com/results?search_query=', '+', message.content, 7));
            message.delete();
        }
        else if (message.content.startsWith('/네이버링크 ')) {
            message.channel.send(link('https://search.naver.com/search.naver?sm=tab_hty.top&where=nexearch&query=', '+', message.content, 7));
            message.delete();
        }
        else if (message.content.startsWith('/다음링크 ')) {
            message.channel.send(link('https://search.daum.net/search?w=tot&DA=YZR&t__nil_searchbox=btn&sug=&sugo=&q=', '+', message.content, 6));
            message.delete();
        }
        else if (message.content.startsWith('/구글링크 ')) {
            message.channel.send(link('https://www.google.co.kr/search?q=', '+', message.content, 6));
            message.delete();
        }
        else if (message.content.startsWith('/나무위키 ')) {
            message.channel.send(link('https://namu.wiki/Search?q=', '%20', message.content, 6));
            message.delete();
        }
        else if (message.content === '//재로그인' && message.author.id === '647736678815105037') {
            relogin = true;
        ...
        else if (message.content === '/도배' && message.channel.startsWith('도배')) {
...
async function execute(content, message, serverQueue, song2) {
    ...
    if (!voiceChannel) return message.channel.send('먼저 음성 채널에 들어가주세요');
...
function skip(message, serverQueue) {
    if (!message.member.voiceChannel) return message.channel.send('음성 채널에 들어가주세요');
    if (!serverQueue) return message.channel.send('현재 플레이 중인 노래가 없어요');
